...
def SystemInfo():
    ...
    Systeminfo['BootTime'] = time.strftime(
        '%Y-%m-%d %H:%M:%S',
        time.localtime(
            psutil.boot_time()))
...
def ProcessInfo(PName):
    ...
    def PidInfo(ProcList):
        ...
        # pidDic['PidIOWrite'] = bytes2human(
        #     newP_io.write_bytes - p_io.write_bytes)
        pidDic['PidCmdLine'] = " ".join(p.cmdline())
        returnDic[str(ProcList['name']) + '-pid-' +
                  str(ProcList['pid'])] = pidDic
